...
public class WeakValueHashMap<K, V> implements Map<K, V> {
	...
	private static class WeakValue<K, V> extends WeakReference<V> {
		 ...
		 */
		static <K, V> WeakValue<K, V> create(final V value) {
			if (value == null)
				return null;
			else
		 ...
		 */
		static <K, V> WeakValue<K, V> create(final K key, final V value, final ReferenceQueue<V> queue) {
			if (value == null)
				return null;
			else
		...
		public boolean equals(final Object obj) {
			...
			@SuppressWarnings("unchecked")
			Object ref2 = ((WeakValue<K, V>) obj).get();
